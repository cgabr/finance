...
t0         = 0
max_months = 4   #  10^max_months Monate erlaubt in einem sum-file, 10^4/12 = 800 Jahre
                 #      (das sollte reichen)
...
#    def format_sort (x):                                              #   In der result.orig-Datei stehen die
#        if x in sortierte_zeilen:                                     #   gematchten Buchungssaetze als Direktkopie
#            return(sortierte_zeilen.index(x))                         #   aus den acc-files.
#        else:
#            return(99999999)                                          #   Die formatierte Original-Version dieser
                                                                      #   result.orig-Datei ist dann result.format.
    diff_salden              = {}
...
#  ---  2. Jetzt zusammensetzen
    ...
    if os.path.isfile(file+".par"):
        ...
        if m:                           #  In der result.par stehen die urspruenglichen Request-Daten.
            ...
            if interval == "None":
                interval = None       
            split_subtract([startdate,enddate,pattern,interval,egrep],file)
            ...
            if m:
                ...
                if datum.startswith(abschnitt):
                    ...
                    if m.group(1).startswith("<"):
                        betrag = -betrag
                    compute_salden(diff_salden,is_buchung,ktoa, betrag,datum) 
                    compute_salden(diff_salden,is_buchung,ktob,-betrag,datum)
...
#                    print(werte)
                ...
                if len(werte) > 0:
                    ...
                    nr0     = 0
                    for betrag in werte:
                        nr     = int(betrag[-max_months:])
                        betrag = float(betrag[:-max_months])
            ...
            if len(betraege) == 1 and float(betraege[0][:-max_months]) == 0.00:
                if interval9 > -1:
...
def compute_salden (salden,buchung,ukto,betrag,datum):
...
#    os.system("cp " + file + " " + file+".xxx")
#    exit()
#    startdate = os.popen("head -n 1 " + file).read()
#    enddate   = os.popen("tail -n 1 " + file).read()
#    print("QQQQQ",file,startdate,enddate)
    startdate = os.popen("head -n 1 " + file).read()[0:8]
    enddate   = os.popen("tail -n 1 " + file).read()[0:8]
#    print("WWWWW",startdate,enddate)
...
def split (pattern="",file="result"):

    if os.path.isfile(file+".orig"):
        add()
        return()

    split_pars = split1(pattern,file)
    split_subtract(split_pars,file)

#******************************************************************************

def split1 (pattern="",file="result"):

    mark("")
    pattern1 = re.sub(r"\^"," ",pattern,9999)
    (pattern1,interval,egrep) = parse_pattern(pattern1)

    mark("A. Compute query for transaction file " + pattern1 + " " + str(interval)[0:40] + " ...")
    if interval:
        if pattern1 == "":
...
#        open(file+".orig","w").write("\n".join(text1)+"\n")
    ...
    mark("B. Transaction file generated.")

    if len(glob.glob("*.acc")) == len(glob.glob("*.sum")):
        format_kto(pattern1,interval,pattern)
        mark("C. ... and formatted.")
    
    return([startdate,enddate,pattern1,interval,egrep])

#******************************************************************************
    
def split_subtract (pars,file="result"): 
...
def format_kto (pattern,interval,pattern0,file="result"):
...
#        print(zeile1)
    ...
    salden_text = format_salden(ukto,datum,datum0,"^"+ukto+"(-[^- ]+){0,99},")
...
def apply_patch (file,patch,offset=0):  #  verbessern! Kann der patch Befehl auch direkt auf result.txt angewendet werden?
...
def format_salden (kto0,datum,start_datum="00000000",ktoparse="^[^- ]+-[^- ]+ "):
        ...
        if len(ktotexts[0]) > 0 and kto == ktotexts[0][0][0]:   #  im ersten Salden-File die genaue Monatsspalte finden
            ...
            if start_datum >= datum0:
                ...
                if datpos < 0:
                    ...
                    betrag = betrag - float(zeile[-1][:-max_months])
                    nr     = nr     - int(zeile[-1][-max_months:])
                else:
                    betrag = betrag - float(zeile[datpos][:-max_months])
                    nr     = nr     - int(zeile[datpos][-max_months:])
#                if kto == "13":
...
#    print("PP",pattern,ktofile)
#    print("UDIR",udir)

    split_pars = split1(pattern)
 #    split_subtract(split_pars)
