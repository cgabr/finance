...
file = "99_bilanzen_old/xx.sh"
text1 = '''
#a2ps -B -1 -l 80 -o bilanz_$1_$2_hauptteil.ps $1_*md
a2ps -B -1 -l 172 -r -o bilanz_$1_$2_hauptteil.ps $1_*md
ps2pdf bilanz_$1_$2_hauptteil.ps
#a2ps -B -1 -l 150 -r -o bilanz_$1_$2_zusaetze.ps anlagen*$1.md
a2ps -B -1 -l 172 -r -o bilanz_$1_$2_zusaetze.ps anlagen*$1.md
ps2pdf bilanz_$1_$2_zusaetze.ps
rm *.ps
rm *.ps~
'''
...
while (0 == 0):
    ...
    buch = re.sub(r"^(.*)[\\\/](.*)$","\\2",os.path.abspath(".") )
    name = { "ift" : "IfT Informatik GmbH",
             "rea" : "Immobilien Christian Gabriel",
             "bbp" : "Bebop Restaurant Christian Gabriel",
             "mus" : "Musik Christian Gabriel",
             "gbr" : "Software Christian Gabriel"
                     }[buch]
    bez  = { "ift" : "ift_gmbh__218_129_10296",
             "rea" : "real_estate_christian_gabriel__218_219_00112",
             "bbp" : "bebop_restaurant__B3174_04701",
             "mus" : "musik_christian_gabriel__218_219_00112",
             "gbr" : "software_christian_gabriel__218_219_00112"
                     }[buch]

    text = "BILANZ " +  name + ", Jahr 20" + jahr
#    text = text + "\n" + re.sub(r".","=",text) #  + "\n\n"

    os.system("rm ggg/*kto; yy1 ggg ^10-:"+jahr)
    text = text + "\nXXAKTIVA   " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()

    os.system("rm ggg/*kto; yy1 ggg ^11-:"+jahr)
    text1 = "\nXXPASSIVA  " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()
    text1 = re.sub(r" (\-?\d+\.\d\d)","-\\1",text1,999999)
    text1 = re.sub(r" --","   ",text1,999999)
    text  = text + text1

    os.system("rm ggg/*kto; yy1 ggg ^12-."+jahr)
    einnahmen = open(glob.glob("ggg/*kto")[0]).read()
    text1 = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\f\nXXERTRAG " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()
    text1 = re.sub(r" (\-?\d+\.\d\d)","-\\1",text1,999999)  #  Minuszeichen
    text1 = re.sub(r" --","   ",text1,999999)
    text  = text + text1

    os.system("rm ggg/*kto; yy1 ggg ^13-."+jahr)
    ausgaben = open(glob.glob("ggg/*kto")[0]).read()
    text  = text + "\nXXAUFWAND" + re.sub(r"Do","Xo",os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read(),99999999)
    ...
    for monat in "123456789ABC"*WITH_BWA:
        os.system("rm ggg/*kto; yy1 ggg ^10-:"+jahr+monat)
        o1 = "\nAKTIVA   " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()
#        o1 = open(glob.glob("ggg/*kto")[0]).read()
        
        os.system("rm ggg/*kto; yy1 ggg ^11-:"+jahr+monat)
        o2 = "\nPASSIVA   " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()
#        o2 = open(glob.glob("ggg/*kto")[0]).read()
        o2 = re.sub(r" (\-?\d+\.\d\d)","-\\1",o2,999999)  #  Minuszeichen
        o2 = re.sub(r" --","   ",o2,999999)
        
        os.system("rm ggg/*kto; yy1 ggg ^12-."+jahr+monat)
        o3 = "\nERTRAG   " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()
#        o3 = open(glob.glob("ggg/*kto")[0]).read()
        o3 = re.sub(r" (\-?\d+\.\d\d)","-\\1",o3,999999)  #  Minuszeichen
        o3 = re.sub(r" --","   ",o3,999999)
        
        os.system("rm ggg/*kto; yy1 ggg ^13-."+jahr+monat)
        o4 = "\nAUFWAND   " + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()
        o4 = re.sub(r"Do","Xo",o4,999999)
        monat_guv.append( o1 + o2 + o3 + o4 ) # open(glob.glob("ggg/*kto")[0]).read() )

    text = re.sub(r"( .\d+\.\d\d)","                        \\1",text,99999999)
    text = re.sub(r"\nXX([A-Z]+)        ([^\n]+)","\n\n\n\\1\\2\n=======",text,9999)

    os.system("rm ggg/*kto; yy1 ggg ^14-."+jahr)
    text  = text + "\n\n\f\nABSCHLUSS" + os.popen("grep -P '^(\S+|     ) +(\S+) *$' ggg/*kto").read()

    text = re.sub(r"( .\d+\.\d\d)","                        \\1",text,99999999)
    text = re.sub(r"\nXX([A-Z]+)        ([^\n]+)","\n\n\n\\1\\2\n=======\n",text,9999)

    os.system("rm ggg/*kto; yy1 ggg ^10-A11-."+jahr)
    anlagen = open(glob.glob("ggg/*kto")[0]).read()

    os.system("rm ggg/*kto; yy1 ggg ^11-C13-3060."+jahr)
    ust = open(glob.glob("ggg/*kto")[0]).read()
...
#                zeile1 = zeile1 + "\n" + ("-" * len(zeile1)) # m.group(1))
            zeile1 = zeile1 + "                                    "
            zeile1 = zeile1[0:75] + m.group(3)[-12*FORMAT2:]
    ...
    if WITH_BWA == 1:
        ...
        for mm in "JA FE MR AP MA JN JL AU SE OC NO DE".split(" "):
            text_monate = text_monate + (FORMAT3 % mm)
#        text1 = re.sub(r"(AUFWAND +\-?\d+\.\d\d)","\\1            "  + text_monate,text1)
        text1 = re.sub(r"ANLAGEVERMOEGEN",  "ANLAGEVERMOEGEN  " + " "*58 + text_monate,text1)
        text1 = re.sub(r"STAMMKAPITAL",     "STAMMKAPITAL     " + " "*58 + text_monate,text1)
        text1 = re.sub(r"ERLOESE",          "ERLOESE          " + " "*58 + text_monate,text1)
        text1 = re.sub(r"PERSONALAUFWAND",  "PERSONALAUFWAND  " + " "*58 + text_monate,text1)

    file = "99_bilanzen_old/"+jahr + "_bilanz_"+bez+"__20" + jahr
    open(file + ".md","w").write(text1)
#    os.system("rm "+file+".p*")



    file = "99_bilanzen_old/anlagen_einnahmen_ausgaben_umsatz_vorsteuer_"+bez+"__20" + jahr
    ...
    os.system("chdir 99_bilanzen_old; sh xx.sh " + jahr + " " + bez + "; chdir ..")
