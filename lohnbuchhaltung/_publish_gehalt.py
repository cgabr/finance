...
#  1. Gehaltsbescheinigung erstellen
        ...
        if len(x) == 0: # aber nur, wenn es noch keine Gehaltsbescheinigung gibt
            print(p,monat1)
            os.system("cd " + p + "; python3 -m _lohn; python3 -m _lohn " + monat)
            ...
            pp[p] = 1
            os.system("cd " + p + "; python3 -m _konto kto")  #  die urspruengliche Form des Kontos wiederherstellen
            new_gehalt = 1
            time.sleep(0.1)
...
#            print( "ln -s " + os.path.abspath(p+"/52_sozialversicherungsmeldungen") + " " + ablage)
            if not os.path.isdir(ablage+"/52_sozialversicherungsmeldungen"):
                os.system( "ln -s " + os.path.abspath(p+"/52_sozialversicherungsmeldungen") + " " + ablage)
            if m:
