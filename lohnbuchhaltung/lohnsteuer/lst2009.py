...
class Lst2009:
    ...
    def MAIN(self):
        ...
        self.JW = self.LSTJAHR.multiply(Lst2009.ZAHL100)
        self.UPANTEIL()
    ...
    def MRE4JL(self):
        ...
        """
        if self.LZZ == 1:
            self.ZRE4J = self.RE4.divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.VBEZ.divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLFREIB = self.LZZFREIB.divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLHINZU = self.LZZHINZU.divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        else:
            if self.LZZ == 2:
                self.ZRE4J = self.RE4.multiply(Lst2009.ZAHL12).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.ZVBEZJ = self.VBEZ.multiply(Lst2009.ZAHL12).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLFREIB = self.LZZFREIB.multiply(Lst2009.ZAHL12).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLHINZU = self.LZZHINZU.multiply(Lst2009.ZAHL12).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ZRE4J = self.RE4.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                else:
                    self.ZRE4J = self.RE4.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2009.ZAHL360).divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
    ...
    def MRE4(self):
        ...
        if self.ZVBEZJ.compareTo(BigDecimal.ZERO) == 0:
            ...
            if self.LZZ == 1:
                ...
                self.HFVB = Lst2009.TAB2[self.J].divide(Lst2009.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB))
                self.FVBZ = Lst2009.TAB3[self.J].divide(Lst2009.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB)).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.VBEZB = self.VBEZM.multiply(Lst2009.ZAHL12).add(self.VBEZS).setScale(2, BigDecimal.ROUND_DOWN)
                self.HFVB = Lst2009.TAB2[self.J]
                self.FVBZ = Lst2009.TAB3[self.J]
            self.FVB = self.VBEZB.multiply(Lst2009.TAB1[self.J]).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_UP)
            if self.FVB.compareTo(self.HFVB) == 1:
                self.FVB = self.HFVB
            self.FVBSO = self.FVB.add(self.VBEZBSO.multiply(Lst2009.TAB1[self.J]).divide(Lst2009.ZAHL100)).setScale(2, BigDecimal.ROUND_UP)
            if self.FVBSO.compareTo(Lst2009.TAB2[self.J]) == 1:
                self.FVBSO = Lst2009.TAB2[self.J]
            self.HFVBZSO = self.VBEZB.add(self.VBEZBSO).divide(Lst2009.ZAHL100).subtract(self.FVBSO).setScale(2, BigDecimal.ROUND_DOWN)
            self.FVBZSO = self.FVBZ.add(self.VBEZBSO.divide(Lst2009.ZAHL100)).setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(self.HFVBZSO) == 1:
                self.FVBZSO = self.HFVBZSO.setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(Lst2009.TAB3[self.J]) == 1:
                self.FVBZSO = Lst2009.TAB3[self.J]
            self.HFVBZ = self.VBEZB.divide(Lst2009.ZAHL100).subtract(self.FVB).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MRE4ALTE(self):
        ...
        if self.ALTER1 == 0:
            ...
            self.ALTE = self.BMG.multiply(Lst2009.TAB4[self.K]).setScale(2, BigDecimal.ROUND_UP)
            self.HBALTE = Lst2009.TAB5[self.K]
    ...
    def MZTABFB(self):
        ...
        if self.STKL == 1:
            ...
            self.KFB = self.ZKF.multiply(BigDecimal.valueOf(6024)).setScale(0, BigDecimal.ROUND_DOWN)
        else:
            if self.STKL == 2:
                ...
                if self.STKL == 3:
                    ...
                    if self.STKL == 4:
                        self.SAP = BigDecimal.valueOf(36)
                        self.KFB = self.ZKF.multiply(BigDecimal.valueOf(3012)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
    ...
    def MLSTJAHR(self):
        ...
        if self.KENNVMT == 0:
            ...
            self.ZVE = self.ZRE4.subtract(self.ZTABFB).subtract(self.VSP).subtract(self.VMT.divide(Lst2009.ZAHL100)).subtract(self.VKAPA.divide(Lst2009.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
            if self.ZVE.compareTo(BigDecimal.ZERO) == -1:
                self.ZVE = self.ZVE.add(self.VMT.divide(Lst2009.ZAHL100)).add(self.VKAPA.divide(Lst2009.ZAHL100)).divide(Lst2009.ZAHL5).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.multiply(Lst2009.ZAHL5).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.UPMLST()
                self.STOVMT = self.ST
                self.ZVE = self.ZVE.add(self.VMT.add(self.VKAPA).divide(Lst2009.ZAHL500)).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.subtract(self.STOVMT).multiply(Lst2009.ZAHL5).add(self.STOVMT).setScale(0, BigDecimal.ROUND_DOWN)

    def UPMLST(self):
        """
        PAP Seite 18 Ermittlung der Jahreslohnsteuer aus dem Einkommensteuertarif
        """
        if self.ZVE.compareTo(Lst2009.ZAHL1) == -1:
    ...
    def UPEVP(self):
        ...
        if self.KRV > 0:
            ...
            if self.ZRE4VP.compareTo(BigDecimal.valueOf(64800)) == 1:
                self.ZRE4VP = BigDecimal.valueOf(64800)
            self.VSP1 = self.ZRE4VP.multiply(BigDecimal.valueOf(0.36)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def UMVSP(self):
        ...
        if self.VSPO.compareTo(self.VSPVOR) == 1:
            ...
            if self.VSPREST.compareTo(self.VSPMAX1) == 1:
                ...
                self.VSPREST = self.VSPREST.subtract(self.VSPMAX1).divide(Lst2009.ZAHL2, 2, BigDecimal.ROUND_UP)
    ...
    def MST5_6(self):
        ...
        """
        self.ZZX = self.X
        if self.ZZX.compareTo(BigDecimal.valueOf(26276)) == 1:
            self.ZX = BigDecimal.valueOf(26276)
            self.UP5_6()
            if self.ZZX.compareTo(BigDecimal.valueOf(200320)) == 1:
                self.ST = self.ST.add(BigDecimal.valueOf(200320).subtract(BigDecimal.valueOf(26276)).multiply(BigDecimal.valueOf(0.42))).setScale(0, BigDecimal.ROUND_DOWN)
                self.ST = self.ST.add(self.ZZX.subtract(BigDecimal.valueOf(200320)).multiply(BigDecimal.valueOf(0.45))).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.ST = self.ST.add(self.ZZX.subtract(BigDecimal.valueOf(26276)).multiply(BigDecimal.valueOf(0.42))).setScale(0, BigDecimal.ROUND_DOWN)
        else:
            self.ZX = self.ZZX
            self.UP5_6()
            if self.ZZX.compareTo(BigDecimal.valueOf(9225)) == 1:
                self.VERGL = self.ST
                self.ZX = BigDecimal.valueOf(9225)
                self.UP5_6()
                self.HOCH = self.ST.add(self.ZZX.subtract(BigDecimal.valueOf(9225)).multiply(BigDecimal.valueOf(0.42))).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def UP5_6(self):
        ...
        self.UPTAB09()
        self.ST2 = self.ST
        self.DIFF = self.ST1.subtract(self.ST2).multiply(Lst2009.ZAHL2)
    ...
    def MSOLZ(self):
        ...
        if self.JBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZJ = self.JBMG.multiply(BigDecimal.valueOf(5.5)).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.SOLZMIN = self.JBMG.subtract(self.SOLZFREI).multiply(BigDecimal.valueOf(20)).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.SOLZMIN.compareTo(self.SOLZJ) == -1:
                self.SOLZJ = self.SOLZMIN
            self.JW = self.SOLZJ.multiply(Lst2009.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            self.UPANTEIL()
            self.SOLZLZZ = self.ANTEIL1
        else:
            self.SOLZLZZ = BigDecimal.ZERO
        if self.R > 0:
            self.JW = self.JBMG.multiply(Lst2009.ZAHL100)
            self.UPANTEIL()
    ...
    def UPANTEIL(self):
        ...
        if self.LZZ == 1:
            ...
            if self.LZZ == 2:
                self.ANTEIL1 = self.JW.divide(Lst2009.ZAHL12, 0, BigDecimal.ROUND_DOWN)
                self.ANTEIL2 = self.JW.divide(Lst2009.ZAHL12, 0, BigDecimal.ROUND_UP)
            else:
                if self.LZZ == 3:
                    self.ANTEIL1 = self.JW.multiply(Lst2009.ZAHL7).divide(Lst2009.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                    self.ANTEIL2 = self.JW.multiply(Lst2009.ZAHL7).divide(Lst2009.ZAHL360, 0, BigDecimal.ROUND_UP)
                else:
                    self.ANTEIL1 = self.JW.divide(Lst2009.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                    self.ANTEIL2 = self.JW.divide(Lst2009.ZAHL360, 0, BigDecimal.ROUND_UP)
    ...
    def MSONST(self):
        ...
        if self.SONSTB.compareTo(BigDecimal.ZERO) == 0:
            ...
            self.MOSONST()
            self.ZRE4J = self.JRE4.add(self.SONSTB).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.VBEZBSO = self.STERBE
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LSTSO = self.ST.multiply(Lst2009.ZAHL100)
            self.STS = self.LSTSO.subtract(self.LSTOSO)
            if self.STS.compareTo(BigDecimal.ZERO) == -1:
                self.STS = BigDecimal.ZERO
            self.SOLZS = self.STS.multiply(BigDecimal.valueOf(5.5)).divide(Lst2009.ZAHL100, 0, BigDecimal.ROUND_DOWN)
            if self.R > 0:
                self.BKS = self.STS
            else:
                self.BKS = BigDecimal.ZERO
    ...
    def MVMT(self):
        ...
        if self.VMT.add(self.VKAPA).compareTo(BigDecimal.ZERO) == 1:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).add(self.VMT).add(self.VKAPA).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).add(self.VKAPA).divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LST3 = self.ST.multiply(Lst2009.ZAHL100)
            self.KENNVMT = 1
            self.MLSTJAHR()
            self.LST2 = self.ST.multiply(Lst2009.ZAHL100)
            ...
            self.SOLZV = self.STV.multiply(BigDecimal.valueOf(5.5)).divide(Lst2009.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            if self.R > 0:
                self.BKV = self.STV
            else:
    ...
    def MOSONST(self):
        ...
        """
        self.ZRE4J = self.JRE4.divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.ZVBEZJ = self.JVBEZ.divide(Lst2009.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.JLFREIB = self.JFREIB.divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.JLHINZU = self.JHINZU.divide(Lst2009.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.MRE4()
        self.MRE4ABZ()
        self.MZTABFB()
        self.MLSTJAHR()
        self.LSTOSO = self.ST.multiply(Lst2009.ZAHL100)
    ...
    def UPTAB09(self):
        ...
        """
        if self.X.compareTo(BigDecimal.valueOf(7835)) == -1:
            self.ST = BigDecimal.ZERO
        else:
            if self.X.compareTo(BigDecimal.valueOf(13140)) == -1:
                self.Y = self.X.subtract(BigDecimal.valueOf(7834)).divide(BigDecimal.valueOf(10000), 6, BigDecimal.ROUND_DOWN)
                self.RW = self.Y.multiply(BigDecimal.valueOf(939.68))
                self.RW = self.RW.add(BigDecimal.valueOf(1400))
                self.ST = self.RW.multiply(self.Y).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                if self.X.compareTo(BigDecimal.valueOf(52552)) == -1:
                    self.Y = self.X.subtract(BigDecimal.valueOf(13139)).divide(BigDecimal.valueOf(10000), 6, BigDecimal.ROUND_DOWN)
                    ...
                    self.ST = self.RW.add(BigDecimal.valueOf(1007)).setScale(0, BigDecimal.ROUND_DOWN)
                else:
                    if self.X.compareTo(BigDecimal.valueOf(250401)) == -1:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.42)).subtract(BigDecimal.valueOf(8064)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.45)).subtract(BigDecimal.valueOf(15576)).setScale(0, BigDecimal.ROUND_DOWN)
