...
class Lst2011c:
    ...
    def MAIN(self):
        ...
        self.JW = self.LSTJAHR.multiply(Lst2011c.ZAHL100)
        self.UPANTEIL()
    ...
    def UPANP12(self):
        ...
        """
        if self.LZZ == 1:
            if self.ZRE4.subtract(self.ZVBEZ).compareTo(BigDecimal.valueOf(1000)) == -1:
                self.ANP = self.ANP.add(self.ZRE4).subtract(self.ZVBEZ).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.ANP = self.ANP.add(BigDecimal.valueOf(1000))
        else:
            if self.ZRE4.subtract(self.ZVBEZ).compareTo(BigDecimal.valueOf(1880)) == -1:
                self.ANP = self.ANP.add(self.ZRE4).subtract(self.ZVBEZ).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.ANP = self.ANP.add(BigDecimal.valueOf(1880))
    ...
    def MRE4JL(self):
        ...
        """
        if self.LZZ == 1:
            self.ZRE4J = self.RE4.divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.VBEZ.divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLFREIB = self.LZZFREIB.divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLHINZU = self.LZZHINZU.divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        else:
            if self.LZZ == 2:
                self.ZRE4J = self.RE4.multiply(Lst2011c.ZAHL12).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.ZVBEZJ = self.VBEZ.multiply(Lst2011c.ZAHL12).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLFREIB = self.LZZFREIB.multiply(Lst2011c.ZAHL12).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLHINZU = self.LZZHINZU.multiply(Lst2011c.ZAHL12).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ZRE4J = self.RE4.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                else:
                    self.ZRE4J = self.RE4.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2011c.ZAHL360).divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        if self.af == 0:
            self.f = 1
    ...
    def MRE4(self):
        ...
        if self.ZVBEZJ.compareTo(BigDecimal.ZERO) == 0:
            ...
            if self.LZZ == 1:
                ...
                self.HFVB = Lst2011c.TAB2[self.J].divide(Lst2011c.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB))
                self.FVBZ = Lst2011c.TAB3[self.J].divide(Lst2011c.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB)).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.VBEZB = self.VBEZM.multiply(Lst2011c.ZAHL12).add(self.VBEZS).setScale(2, BigDecimal.ROUND_DOWN)
                self.HFVB = Lst2011c.TAB2[self.J]
                self.FVBZ = Lst2011c.TAB3[self.J]
            self.FVB = self.VBEZB.multiply(Lst2011c.TAB1[self.J]).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_UP)
            if self.FVB.compareTo(self.HFVB) == 1:
                self.FVB = self.HFVB
            self.FVBSO = self.FVB.add(self.VBEZBSO.multiply(Lst2011c.TAB1[self.J]).divide(Lst2011c.ZAHL100)).setScale(2, BigDecimal.ROUND_UP)
            if self.FVBSO.compareTo(Lst2011c.TAB2[self.J]) == 1:
                self.FVBSO = Lst2011c.TAB2[self.J]
            self.HFVBZSO = self.VBEZB.add(self.VBEZBSO).divide(Lst2011c.ZAHL100).subtract(self.FVBSO).setScale(2, BigDecimal.ROUND_DOWN)
            self.FVBZSO = self.FVBZ.add(self.VBEZBSO.divide(Lst2011c.ZAHL100)).setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(self.HFVBZSO) == 1:
                self.FVBZSO = self.HFVBZSO.setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(Lst2011c.TAB3[self.J]) == 1:
                self.FVBZSO = Lst2011c.TAB3[self.J]
            self.HFVBZ = self.VBEZB.divide(Lst2011c.ZAHL100).subtract(self.FVB).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MRE4ALTE(self):
        ...
        if self.ALTER1 == 0:
            ...
            self.ALTE = self.BMG.multiply(Lst2011c.TAB4[self.K]).setScale(0, BigDecimal.ROUND_UP)
            self.HBALTE = Lst2011c.TAB5[self.K]
    ...
    def MRE4ABZ(self):
        ...
        if self.KENNVMT == 2:
            self.ZRE4VP = self.ZRE4VP.subtract(self.ENTSCH.divide(Lst2011c.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MLSTJAHR(self):
        ...
        if self.KENNVMT != 1:
            ...
            self.ZVE = self.ZRE4.subtract(self.ZTABFB).subtract(self.VSP).subtract(self.VMT.divide(Lst2011c.ZAHL100)).subtract(self.VKAPA.divide(Lst2011c.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
            if self.ZVE.compareTo(BigDecimal.ZERO) == -1:
                self.ZVE = self.ZVE.add(self.VMT.divide(Lst2011c.ZAHL100)).add(self.VKAPA.divide(Lst2011c.ZAHL100)).divide(Lst2011c.ZAHL5).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.multiply(Lst2011c.ZAHL5).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.UPMLST()
                self.STOVMT = self.ST
                self.ZVE = self.ZVE.add(self.VMT.add(self.VKAPA).divide(Lst2011c.ZAHL500)).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.subtract(self.STOVMT).multiply(Lst2011c.ZAHL5).add(self.STOVMT).setScale(0, BigDecimal.ROUND_DOWN)

    def UPMLST(self):
        """
        PAP Seite 18 Ermittlung der Jahreslohnsteuer aus dem Einkommensteuertarif
        """
        if self.ZVE.compareTo(Lst2011c.ZAHL1) == -1:
    ...
    def MVSP(self):
        ...
        if self.PKV > 0:
            if self.STKL == 6:
                ...
                self.VSP = self.PKPV.multiply(Lst2011c.ZAHL12).divide(Lst2011c.ZAHL100)
                if self.PKV == 2:
    ...
    def UMVSP(self):
        ...
        if self.VSPO.compareTo(self.VSPVOR) == 1:
            ...
            if self.VSPREST.compareTo(self.VSPMAX1) == 1:
                ...
                self.VSPREST = self.VSPREST.subtract(self.VSPMAX1).divide(Lst2011c.ZAHL2, 2, BigDecimal.ROUND_UP)
    ...
    def UP5_6(self):
        ...
        self.UPTAB10()
        self.ST2 = self.ST
        self.DIFF = self.ST1.subtract(self.ST2).multiply(Lst2011c.ZAHL2)
    ...
    def MSOLZ(self):
        ...
        if self.JBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZJ = self.JBMG.multiply(BigDecimal.valueOf(5.5)).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.SOLZMIN = self.JBMG.subtract(self.SOLZFREI).multiply(BigDecimal.valueOf(20)).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.SOLZMIN.compareTo(self.SOLZJ) == -1:
                self.SOLZJ = self.SOLZMIN
            self.JW = self.SOLZJ.multiply(Lst2011c.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            self.UPANTEIL()
            self.SOLZLZZ = self.ANTEIL1
        else:
            self.SOLZLZZ = BigDecimal.ZERO
        if self.R > 0:
            self.JW = self.JBMG.multiply(Lst2011c.ZAHL100)
            self.UPANTEIL()
    ...
    def UPANTEIL(self):
        ...
        if self.LZZ == 1:
            ...
            if self.LZZ == 2:
                self.ANTEIL1 = self.JW.divide(Lst2011c.ZAHL12, 0, BigDecimal.ROUND_DOWN)
                self.ANTEIL2 = self.JW.divide(Lst2011c.ZAHL12, 0, BigDecimal.ROUND_UP)
            else:
                if self.LZZ == 3:
                    self.ANTEIL1 = self.JW.multiply(Lst2011c.ZAHL7).divide(Lst2011c.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                    self.ANTEIL2 = self.JW.multiply(Lst2011c.ZAHL7).divide(Lst2011c.ZAHL360, 0, BigDecimal.ROUND_UP)
                else:
                    self.ANTEIL1 = self.JW.divide(Lst2011c.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                    self.ANTEIL2 = self.JW.divide(Lst2011c.ZAHL360, 0, BigDecimal.ROUND_UP)
    ...
    def MSONST(self):
        ...
        if self.SONSTB.compareTo(BigDecimal.ZERO) == 0:
            ...
            self.MOSONST()
            self.ZRE4J = self.JRE4.add(self.SONSTB).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.VBEZBSO = self.STERBE
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LSTSO = self.ST.multiply(Lst2011c.ZAHL100)
            self.STS = self.LSTSO.subtract(self.LSTOSO).multiply(BigDecimal.valueOf(self.f)).divide(Lst2011c.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lst2011c.ZAHL100)
            if self.STS.compareTo(BigDecimal.ZERO) == -1:
                self.STS = BigDecimal.ZERO
            self.SOLZS = self.STS.multiply(BigDecimal.valueOf(5.5)).divide(Lst2011c.ZAHL100, 0, BigDecimal.ROUND_DOWN)
            if self.R > 0:
                self.BKS = self.STS
            else:
                self.BKS = BigDecimal.ZERO
    ...
    def MVMT(self):
        ...
        if self.VMT.add(self.VKAPA).compareTo(BigDecimal.ZERO) == 1:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).add(self.VMT).add(self.VKAPA).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).add(self.VKAPA).divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.KENNVMT = 2
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LST3 = self.ST.multiply(Lst2011c.ZAHL100)
            self.MRE4ABZ()
            self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lst2011c.ZAHL100)).subtract(self.SONSTENT.divide(Lst2011c.ZAHL100))
            self.KENNVMT = 1
            self.MLSTJAHR()
            self.LST2 = self.ST.multiply(Lst2011c.ZAHL100)
            ...
            if self.STV.compareTo(BigDecimal.ZERO) == -1:
                ...
                self.STV = self.STV.multiply(BigDecimal.valueOf(self.f)).divide(Lst2011c.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lst2011c.ZAHL100)
            self.SOLZV = self.STV.multiply(BigDecimal.valueOf(5.5)).divide(Lst2011c.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            if self.R > 0:
                self.BKV = self.STV
            else:
    ...
    def MOSONST(self):
        ...
        """
        self.ZRE4J = self.JRE4.divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.ZVBEZJ = self.JVBEZ.divide(Lst2011c.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.JLFREIB = self.JFREIB.divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.JLHINZU = self.JHINZU.divide(Lst2011c.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.MRE4()
        self.MRE4ABZ()
        self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lst2011c.ZAHL100))
        self.MZTABFB()
        self.MLSTJAHR()
        self.LSTOSO = self.ST.multiply(Lst2011c.ZAHL100)
