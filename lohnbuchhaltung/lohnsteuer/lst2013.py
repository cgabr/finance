...
class Lst2013:
    ...
    RENTBEMESSUNGSGR_OST_2013 = BigDecimal(58800)
    """
    Neuer Wert  2013
    """

    RENTBEMESSUNGSGR_WEST = BigDecimal(69600)
    """
    Neuer Wert 2013
    """
    ...
    def MAIN(self):
        ...
        self.JW = self.LSTJAHR.multiply(Lst2013.ZAHL100)
        self.UPLSTLZZ()
        self.UPVKVLZZ()
    ...
    def MRE4JL(self):
        ...
        """
        if self.LZZ == 1:
            self.ZRE4J = self.RE4.divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.VBEZ.divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLFREIB = self.LZZFREIB.divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLHINZU = self.LZZHINZU.divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        else:
            if self.LZZ == 2:
                self.ZRE4J = self.RE4.multiply(Lst2013.ZAHL12).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.ZVBEZJ = self.VBEZ.multiply(Lst2013.ZAHL12).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLFREIB = self.LZZFREIB.multiply(Lst2013.ZAHL12).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLHINZU = self.LZZHINZU.multiply(Lst2013.ZAHL12).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ZRE4J = self.RE4.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                else:
                    self.ZRE4J = self.RE4.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2013.ZAHL360).divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        if self.af == 0:
            self.f = 1
    ...
    def MRE4(self):
        ...
        if self.ZVBEZJ.compareTo(BigDecimal.ZERO) == 0:
            ...
            if self.LZZ == 1:
                ...
                self.HFVB = Lst2013.TAB2[self.J].divide(Lst2013.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB))
                self.FVBZ = Lst2013.TAB3[self.J].divide(Lst2013.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB)).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.VBEZB = self.VBEZM.multiply(Lst2013.ZAHL12).add(self.VBEZS).setScale(2, BigDecimal.ROUND_DOWN)
                self.HFVB = Lst2013.TAB2[self.J]
                self.FVBZ = Lst2013.TAB3[self.J]
            self.FVB = self.VBEZB.multiply(Lst2013.TAB1[self.J]).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_UP)
            if self.FVB.compareTo(self.HFVB) == 1:
                self.FVB = self.HFVB
            self.FVBSO = self.FVB.add(self.VBEZBSO.multiply(Lst2013.TAB1[self.J]).divide(Lst2013.ZAHL100)).setScale(2, BigDecimal.ROUND_UP)
            if self.FVBSO.compareTo(Lst2013.TAB2[self.J]) == 1:
                self.FVBSO = Lst2013.TAB2[self.J]
            self.HFVBZSO = self.VBEZB.add(self.VBEZBSO).divide(Lst2013.ZAHL100).subtract(self.FVBSO).setScale(2, BigDecimal.ROUND_DOWN)
            self.FVBZSO = self.FVBZ.add(self.VBEZBSO.divide(Lst2013.ZAHL100)).setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(self.HFVBZSO) == 1:
                self.FVBZSO = self.HFVBZSO.setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(Lst2013.TAB3[self.J]) == 1:
                self.FVBZSO = Lst2013.TAB3[self.J]
            self.HFVBZ = self.VBEZB.divide(Lst2013.ZAHL100).subtract(self.FVB).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MRE4ALTE(self):
        ...
        if self.ALTER1 == 0:
            ...
            self.ALTE = self.BMG.multiply(Lst2013.TAB4[self.K]).setScale(0, BigDecimal.ROUND_UP)
            self.HBALTE = Lst2013.TAB5[self.K]
    ...
    def MRE4ABZ(self):
        ...
        if self.KENNVMT == 2:
            self.ZRE4VP = self.ZRE4VP.subtract(self.ENTSCH.divide(Lst2013.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MZTABFB(self):
        ...
        if self.STKL < 6:
            if self.ZRE4.compareTo(self.ZVBEZ) == 1:
                if self.ZRE4.subtract(self.ZVBEZ).compareTo(Lst2013.ZAHL1000) == -1:
                    self.ANP = self.ANP.add(self.ZRE4).subtract(self.ZVBEZ).setScale(0, BigDecimal.ROUND_UP)
                else:
                    self.ANP = self.ANP.add(Lst2013.ZAHL1000)
        self.KZTAB = 1
        if self.STKL == 1:
    ...
    def MLSTJAHR(self):
        ...
        if self.KENNVMT != 1:
            ...
            self.ZVE = self.ZRE4.subtract(self.ZTABFB).subtract(self.VSP).subtract(self.VMT.divide(Lst2013.ZAHL100)).subtract(self.VKAPA.divide(Lst2013.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
            if self.ZVE.compareTo(BigDecimal.ZERO) == -1:
                self.ZVE = self.ZVE.add(self.VMT.divide(Lst2013.ZAHL100)).add(self.VKAPA.divide(Lst2013.ZAHL100)).divide(Lst2013.ZAHL5).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.multiply(Lst2013.ZAHL5).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.UPMLST()
                self.STOVMT = self.ST
                self.ZVE = self.ZVE.add(self.VMT.add(self.VKAPA).divide(Lst2013.ZAHL500)).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.subtract(self.STOVMT).multiply(Lst2013.ZAHL5).add(self.STOVMT).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def UPVKV(self):
        if self.PKV > 0:
            if self.VSP2.compareTo(self.VSP3) == 1:
                self.VKV = self.VSP2.multiply(Lst2013.ZAHL100)
            else:
                self.VKV = self.VSP3.multiply(Lst2013.ZAHL100)
        else:
    ...
    def UPMLST(self):
        ...
        """
        if self.ZVE.compareTo(Lst2013.ZAHL1) == -1:
    ...
    def UPEVP(self):
        ...
        if self.KRV > 1:
            ...
            if self.KRV == 0:
                if self.ZRE4VP.compareTo(Lst2013.RENTBEMESSUNGSGR_WEST) == 1:
                    self.ZRE4VP = Lst2013.RENTBEMESSUNGSGR_WEST
            else:
                if self.ZRE4VP.compareTo(Lst2013.RENTBEMESSUNGSGR_OST_2013) == 1:
                    self.ZRE4VP = Lst2013.RENTBEMESSUNGSGR_OST_2013
            self.VSP1 = self.ZRE4VP.multiply(BigDecimal.valueOf(0.52)).setScale(2, BigDecimal.ROUND_DOWN)
            self.VSP1 = self.VSP1.multiply(BigDecimal.valueOf(0.0945)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MVSP(self):
        ...
        """
        if self.ZRE4VP.compareTo(BigDecimal.valueOf(47250)) == 1:
            self.ZRE4VP = BigDecimal.valueOf(47250)
        if self.PKV > 0:
            if self.STKL == 6:
                self.VSP3 = BigDecimal.ZERO
            else:
                self.VSP3 = self.PKPV.multiply(Lst2013.ZAHL12).divide(Lst2013.ZAHL100)
                if self.PKV == 2:
                    self.KVSATZAG = BigDecimal.valueOf(0.07).setScale(5)
                    if self.PVS == 1:
                        self.PVSATZAG = BigDecimal.valueOf(0.00525).setScale(5)
                    else:
                        self.PVSATZAG = BigDecimal.valueOf(0.01025).setScale(5)
            ...
            if self.PVS == 1:
                self.PVSATZAN = BigDecimal.valueOf(0.01525).setScale(5)
            else:
                self.PVSATZAN = BigDecimal.valueOf(0.01025).setScale(5)
            if self.PVZ == 1:
    ...
    def UMVSP(self):
        ...
        if self.VSPO.compareTo(self.VSPVOR) == 1:
            ...
            if self.VSPREST.compareTo(self.VSPMAX1) == 1:
                ...
                self.VSPREST = self.VSPREST.subtract(self.VSPMAX1).divide(Lst2013.ZAHL2, 2, BigDecimal.ROUND_UP)
    ...
    def MST5_6(self):
        ...
        if self.ZZX.compareTo(BigDecimal.valueOf(26441)) == 1:
            ...
            self.ZX = self.ZZX
            self.UP5_6()
            if self.ZZX.compareTo(BigDecimal.valueOf(9550)) == 1:
                self.VERGL = self.ST
                self.ZX = BigDecimal.valueOf(9550)
                self.UP5_6()
                self.HOCH = self.ST.add(self.ZZX.subtract(BigDecimal.valueOf(9550)).multiply(BigDecimal.valueOf(0.42))).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def UP5_6(self):
        ...
        self.UPTAB13()
        self.ST2 = self.ST
        self.DIFF = self.ST1.subtract(self.ST2).multiply(Lst2013.ZAHL2)
    ...
    def MSOLZ(self):
        ...
        if self.JBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZJ = self.JBMG.multiply(BigDecimal.valueOf(5.5)).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.SOLZMIN = self.JBMG.subtract(self.SOLZFREI).multiply(BigDecimal.valueOf(20)).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.SOLZMIN.compareTo(self.SOLZJ) == -1:
                self.SOLZJ = self.SOLZMIN
            self.JW = self.SOLZJ.multiply(Lst2013.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            self.UPANTEIL()
            self.SOLZLZZ = self.ANTEIL1
        else:
            self.SOLZLZZ = BigDecimal.ZERO
        if self.R > 0:
            self.JW = self.JBMG.multiply(Lst2013.ZAHL100)
            self.UPANTEIL()
    ...
    def UPANTEIL(self):
        ...
        if self.LZZ == 1:
            ...
            if self.LZZ == 2:
                self.ANTEIL1 = self.JW.divide(Lst2013.ZAHL12, 0, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ANTEIL1 = self.JW.multiply(Lst2013.ZAHL7).divide(Lst2013.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                else:
                    self.ANTEIL1 = self.JW.divide(Lst2013.ZAHL360, 0, BigDecimal.ROUND_DOWN)
    ...
    def MSONST(self):
        ...
        if self.SONSTB.compareTo(BigDecimal.ZERO) == 0:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.VBEZBSO = self.STERBE
            self.MRE4SONST()
            self.MLSTJAHR()
            self.UPVKV()
            self.VKVSONST = self.VKV.subtract(self.VKVSONST)
            self.LSTSO = self.ST.multiply(Lst2013.ZAHL100)
            self.STS = self.LSTSO.subtract(self.LSTOSO).multiply(BigDecimal.valueOf(self.f)).divide(Lst2013.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lst2013.ZAHL100)
            if self.STS.compareTo(BigDecimal.ZERO) == -1:
                self.STS = BigDecimal.ZERO
            self.SOLZS = self.STS.multiply(BigDecimal.valueOf(5.5)).divide(Lst2013.ZAHL100, 0, BigDecimal.ROUND_DOWN)
            if self.R > 0:
                self.BKS = self.STS
            else:
                self.BKS = BigDecimal.ZERO
    ...
    def MVMT(self):
        ...
        if self.VMT.add(self.VKAPA).compareTo(BigDecimal.ZERO) == 1:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).add(self.VMT).add(self.VKAPA).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).add(self.VKAPA).divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.KENNVMT = 2
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LST3 = self.ST.multiply(Lst2013.ZAHL100)
            self.MRE4ABZ()
            self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lst2013.ZAHL100)).subtract(self.SONSTENT.divide(Lst2013.ZAHL100))
            self.KENNVMT = 1
            self.MLSTJAHR()
            self.LST2 = self.ST.multiply(Lst2013.ZAHL100)
            ...
            if self.STV.compareTo(BigDecimal.ZERO) == -1:
                ...
                self.STV = self.STV.multiply(BigDecimal.valueOf(self.f)).divide(Lst2013.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lst2013.ZAHL100)
            self.SOLZV = self.STV.multiply(BigDecimal.valueOf(5.5)).divide(Lst2013.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            if self.R > 0:
                self.BKV = self.STV
            else:
    ...
    def MOSONST(self):
        ...
        """
        self.ZRE4J = self.JRE4.divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.ZVBEZJ = self.JVBEZ.divide(Lst2013.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.JLFREIB = self.JFREIB.divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.JLHINZU = self.JHINZU.divide(Lst2013.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.MRE4()
        self.MRE4ABZ()
        self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lst2013.ZAHL100))
        self.MZTABFB()
        self.MLSTJAHR()
        self.LSTOSO = self.ST.multiply(Lst2013.ZAHL100)
    ...
    def UPTAB13(self):
        ...
        """
        if self.X.compareTo(BigDecimal.valueOf(8131)) == -1:
            self.ST = BigDecimal.ZERO
        else:
            if self.X.compareTo(BigDecimal.valueOf(13470)) == -1:
                self.Y = self.X.subtract(BigDecimal.valueOf(8130)).divide(BigDecimal.valueOf(10000), 6, BigDecimal.ROUND_DOWN)
                self.RW = self.Y.multiply(BigDecimal.valueOf(933.7))
                ...
                if self.X.compareTo(BigDecimal.valueOf(52882)) == -1:
                    ...
                    self.ST = self.RW.add(BigDecimal.valueOf(1014)).setScale(0, BigDecimal.ROUND_DOWN)
                else:
                    if self.X.compareTo(BigDecimal.valueOf(250731)) == -1:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.42)).subtract(BigDecimal.valueOf(8196)).setScale(0, BigDecimal.ROUND_DOWN)
                    else:
                        self.ST = self.X.multiply(BigDecimal.valueOf(0.45)).subtract(BigDecimal.valueOf(15718)).setScale(0, BigDecimal.ROUND_DOWN)
