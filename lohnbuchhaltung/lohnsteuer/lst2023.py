...
class Lst2023:
    ...
    def __init__(self, **kwargs):
        ...
        # 2023 4.008 Euro. Der Erhöhungsbetrag von 2.100 Euro, der für die
        # Jahre 2020 und 2021 galt, ist ab 2023 weggefallen (Jahressteuergesetz 2020).
    ...
    def MPARA(self):
        ...
        self.KVSATZAN = self.KVZ.divide(self.bd).divide(Lst2023.ZAHL100).add(BigDecimal.valueOf(0.07))
    ...
    def MRE4JL(self):
        ...
        """
        if self.LZZ == 1:
            self.ZRE4J = self.RE4.divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.VBEZ.divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLFREIB = self.LZZFREIB.divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            self.JLHINZU = self.LZZHINZU.divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        else:
            if self.LZZ == 2:
                self.ZRE4J = self.RE4.multiply(Lst2023.ZAHL12).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.ZVBEZJ = self.VBEZ.multiply(Lst2023.ZAHL12).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLFREIB = self.LZZFREIB.multiply(Lst2023.ZAHL12).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                self.JLHINZU = self.LZZHINZU.multiply(Lst2023.ZAHL12).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ZRE4J = self.RE4.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL700, 2, BigDecimal.ROUND_DOWN)
                else:
                    self.ZRE4J = self.RE4.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.ZVBEZJ = self.VBEZ.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLFREIB = self.LZZFREIB.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
                    self.JLHINZU = self.LZZHINZU.multiply(Lst2023.ZAHL360).divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        if self.af == 0:
            self.f = 1
    ...
    def MRE4(self):
        ...
        if self.ZVBEZJ.compareTo(BigDecimal.ZERO) == 0:
            ...
            if self.LZZ == 1:
                ...
                self.HFVB = Lst2023.TAB2[self.J].divide(Lst2023.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB))
                self.FVBZ = Lst2023.TAB3[self.J].divide(Lst2023.ZAHL12).multiply(BigDecimal.valueOf(self.ZMVB)).setScale(0, BigDecimal.ROUND_UP)
            else:
                self.VBEZB = self.VBEZM.multiply(Lst2023.ZAHL12).add(self.VBEZS).setScale(2, BigDecimal.ROUND_DOWN)
                self.HFVB = Lst2023.TAB2[self.J]
                self.FVBZ = Lst2023.TAB3[self.J]
            self.FVB = self.VBEZB.multiply(Lst2023.TAB1[self.J]).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_UP)
            if self.FVB.compareTo(self.HFVB) == 1:
                self.FVB = self.HFVB
            if self.FVB.compareTo(self.ZVBEZJ) == 1:
                self.FVB = self.ZVBEZJ
            self.FVBSO = self.FVB.add(self.VBEZBSO.multiply(Lst2023.TAB1[self.J]).divide(Lst2023.ZAHL100)).setScale(2, BigDecimal.ROUND_UP)
            if self.FVBSO.compareTo(Lst2023.TAB2[self.J]) == 1:
                self.FVBSO = Lst2023.TAB2[self.J]
            self.HFVBZSO = self.VBEZB.add(self.VBEZBSO).divide(Lst2023.ZAHL100).subtract(self.FVBSO).setScale(2, BigDecimal.ROUND_DOWN)
            self.FVBZSO = self.FVBZ.add(self.VBEZBSO.divide(Lst2023.ZAHL100)).setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(self.HFVBZSO) == 1:
                self.FVBZSO = self.HFVBZSO.setScale(0, BigDecimal.ROUND_UP)
            if self.FVBZSO.compareTo(Lst2023.TAB3[self.J]) == 1:
                self.FVBZSO = Lst2023.TAB3[self.J]
            self.HFVBZ = self.VBEZB.divide(Lst2023.ZAHL100).subtract(self.FVB).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MRE4ALTE(self):
        ...
        if self.ALTER1 == 0:
            ...
            self.ALTE = self.BMG.multiply(Lst2023.TAB4[self.K]).setScale(0, BigDecimal.ROUND_UP)
            self.HBALTE = Lst2023.TAB5[self.K]
    ...
    def MRE4ABZ(self):
        ...
        if self.KENNVMT == 2:
            self.ZRE4VP = self.ZRE4VP.subtract(self.ENTSCH.divide(Lst2023.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
    ...
    def MBERECH(self):
        ...
        """
        self.MZTABFB()
        self.VFRB = self.ANP.add(self.FVB.add(self.FVBZ)).multiply(Lst2023.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
        self.MLSTJAHR()
        self.WVFRB = self.ZVE.subtract(self.GFB).multiply(Lst2023.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def MZTABFB(self):
        ...
        if self.STKL < 6:
            if self.ZRE4.compareTo(self.ZVBEZ) == 1:
                if self.ZRE4.subtract(self.ZVBEZ).compareTo(Lst2023.ZAHL1000) == -1:
                    self.ANP = self.ANP.add(self.ZRE4).subtract(self.ZVBEZ).setScale(0, BigDecimal.ROUND_UP)
                else:
                    self.ANP = self.ANP.add(Lst2023.ZAHL1000)
        self.KZTAB = 1
        if self.STKL == 1:
    ...
    def MLSTJAHR(self):
        ...
        if self.KENNVMT != 1:
            ...
            self.ZVE = self.ZRE4.subtract(self.ZTABFB).subtract(self.VSP).subtract(self.VMT.divide(Lst2023.ZAHL100)).subtract(self.VKAPA.divide(Lst2023.ZAHL100)).setScale(2, BigDecimal.ROUND_DOWN)
            if self.ZVE.compareTo(BigDecimal.ZERO) == -1:
                self.ZVE = self.ZVE.add(self.VMT.divide(Lst2023.ZAHL100)).add(self.VKAPA.divide(Lst2023.ZAHL100)).divide(Lst2023.ZAHL5).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.multiply(Lst2023.ZAHL5).setScale(0, BigDecimal.ROUND_DOWN)
            else:
                self.UPMLST()
                self.STOVMT = self.ST
                self.ZVE = self.ZVE.add(self.VMT.add(self.VKAPA).divide(Lst2023.ZAHL500)).setScale(2, BigDecimal.ROUND_DOWN)
                self.UPMLST()
                self.ST = self.ST.subtract(self.STOVMT).multiply(Lst2023.ZAHL5).add(self.STOVMT).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def UPVKV(self):
        ...
        if self.PKV > 0:
            if self.VSP2.compareTo(self.VSP3) == 1:
                self.VKV = self.VSP2.multiply(Lst2023.ZAHL100)
            else:
                self.VKV = self.VSP3.multiply(Lst2023.ZAHL100)
        else:
    ...
    def UPLSTLZZ(self):
        ...
        PAP Seite 25
        """
        self.JW = self.LSTJAHR.multiply(Lst2023.ZAHL100)
        self.UPANTEIL()
    ...
    def UPMLST(self):
        ...
        """
        if self.ZVE.compareTo(Lst2023.ZAHL1) == -1:
    ...
    def MVSP(self):
        ...
        if self.PKV > 0:
            if self.STKL == 6:
                ...
                self.VSP3 = self.PKPV.multiply(Lst2023.ZAHL12).divide(Lst2023.ZAHL100)
                if self.PKV == 2:
    ...
    def UP5_6(self):
        ...
        self.UPTAB22()
        self.ST2 = self.ST
        self.DIFF = self.ST1.subtract(self.ST2).multiply(Lst2023.ZAHL2)
    ...
    def MSOLZ(self):
        ...
        if self.JBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZJ = self.JBMG.multiply(BigDecimal.valueOf(5.5)).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.SOLZMIN = self.JBMG.subtract(self.SOLZFREI).multiply(BigDecimal.valueOf(11.9)).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.SOLZMIN.compareTo(self.SOLZJ) == -1:
                self.SOLZJ = self.SOLZMIN
            self.JW = self.SOLZJ.multiply(Lst2023.ZAHL100).setScale(0, BigDecimal.ROUND_DOWN)
            self.UPANTEIL()
            self.SOLZLZZ = self.ANTEIL1
        else:
            self.SOLZLZZ = BigDecimal.ZERO
        if self.R > 0:
            self.JW = self.JBMG.multiply(Lst2023.ZAHL100)
            self.UPANTEIL()
    ...
    def UPANTEIL(self):
        ...
        if self.LZZ == 1:
            ...
            if self.LZZ == 2:
                self.ANTEIL1 = self.JW.divide(Lst2023.ZAHL12, 0, BigDecimal.ROUND_DOWN)
            else:
                if self.LZZ == 3:
                    self.ANTEIL1 = self.JW.multiply(Lst2023.ZAHL7).divide(Lst2023.ZAHL360, 0, BigDecimal.ROUND_DOWN)
                else:
                    self.ANTEIL1 = self.JW.divide(Lst2023.ZAHL360, 0, BigDecimal.ROUND_DOWN)
    ...
    def MSONST(self):
        ...
        if self.SONSTB.compareTo(BigDecimal.ZERO) == 0 and self.MBV.compareTo(BigDecimal.ZERO) == 0:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.VBEZBSO = self.STERBE
            self.MRE4SONST()
            self.MLSTJAHR()
            self.WVFRBM = self.ZVE.subtract(self.GFB).multiply(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            if self.WVFRBM.compareTo(BigDecimal.ZERO) == -1:
                self.WVFRBM = BigDecimal.ZERO
            self.UPVKV()
            self.VKVSONST = self.VKV.subtract(self.VKVSONST)
            self.LSTSO = self.ST.multiply(Lst2023.ZAHL100)
            self.STS = self.LSTSO.subtract(self.LSTOSO).multiply(BigDecimal.valueOf(self.f)).divide(Lst2023.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lst2023.ZAHL100)
            self.STSMIN()
    ...
    def STSMIN(self):
        ...
        if self.STS.compareTo(BigDecimal.ZERO) == -1:
            if self.MBV.compareTo(BigDecimal.ZERO) == 0:
                ...
                self.SOLZLZZ = self.SOLZLZZ.add(self.STS.multiply(BigDecimal.valueOf(5.5).divide(Lst2023.ZAHL100))).setScale(0, BigDecimal.ROUND_DOWN)
    ...
    def MSOLZSTS(self):
        ...
        if self.SOLZSBMG.compareTo(self.SOLZFREI) == 1:
            self.SOLZS = self.STS.multiply(BigDecimal.valueOf(5.5)).divide(Lst2023.ZAHL100, 0, BigDecimal.ROUND_DOWN)
        else:
            self.SOLZS = BigDecimal.ZERO
    ...
    def MVMT(self):
        ...
        if self.VMT.add(self.VKAPA).compareTo(BigDecimal.ZERO) == 1:
            ...
            self.ZRE4J = self.JRE4.add(self.SONSTB).add(self.VMT).add(self.VKAPA).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.ZVBEZJ = self.JVBEZ.add(self.VBS).add(self.VKAPA).divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
            self.KENNVMT = 2
            self.MRE4SONST()
            self.MLSTJAHR()
            self.LST3 = self.ST.multiply(Lst2023.ZAHL100)
            self.MRE4ABZ()
            self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lst2023.ZAHL100)).subtract(self.SONSTENT.divide(Lst2023.ZAHL100))
            self.KENNVMT = 1
            self.MLSTJAHR()
            self.LST2 = self.ST.multiply(Lst2023.ZAHL100)
            ...
            if self.STV.compareTo(BigDecimal.ZERO) == -1:
                ...
                self.STV = self.STV.multiply(BigDecimal.valueOf(self.f)).divide(Lst2023.ZAHL100, 0, BigDecimal.ROUND_DOWN).multiply(Lst2023.ZAHL100)
            self.SOLZVBMG = self.STV.divide(Lst2023.ZAHL100, 0, BigDecimal.ROUND_DOWN).add(self.JBMG)
            if self.SOLZVBMG.compareTo(self.SOLZFREI) == 1:
                self.SOLZV = self.STV.multiply(BigDecimal.valueOf(5.5)).divide(Lst2023.ZAHL100, 0, BigDecimal.ROUND_DOWN)
            else:
    ...
    def MOSONST(self):
        ...
        """
        self.ZRE4J = self.JRE4.divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.ZVBEZJ = self.JVBEZ.divide(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.JLFREIB = self.JFREIB.divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.JLHINZU = self.JHINZU.divide(Lst2023.ZAHL100, 2, BigDecimal.ROUND_DOWN)
        self.MRE4()
        self.MRE4ABZ()
        self.ZRE4VP = self.ZRE4VP.subtract(self.JRE4ENT.divide(Lst2023.ZAHL100))
        self.MZTABFB()
        self.VFRBS1 = self.ANP.add(self.FVB.add(self.FVBZ)).multiply(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        self.MLSTJAHR()
        self.WVFRBO = self.ZVE.subtract(self.GFB).multiply(Lst2023.ZAHL100).setScale(2, BigDecimal.ROUND_DOWN)
        if self.WVFRBO.compareTo(BigDecimal.ZERO) == -1:
            self.WVFRBO = BigDecimal.ZERO
        self.LSTOSO = self.ST.multiply(Lst2023.ZAHL100)
    ...
    def MRE4SONST(self):
        ...
        self.FVB = self.FVBSO
        self.MRE4ABZ()
        self.ZRE4VP = self.ZRE4VP.add(self.MBV.divide(Lst2023.ZAHL100)).subtract(self.JRE4ENT.divide(Lst2023.ZAHL100)).subtract(self.SONSTENT.divide(Lst2023.ZAHL100))
        self.FVBZ = self.FVBZSO
        self.MZTABFB()
        self.VFRBS2 = self.ANP.add(self.FVB).add(self.FVBZ).multiply(Lst2023.ZAHL100).subtract(self.VFRBS1)

    def UPTAB22(self):
        """
        Tarifliche Einkommensteuer §32a EStG, PAP Seite 38
        """
        if self.X.compareTo(self.GFB.add(Lst2023.ZAHL1)) == -1:
            self.ST = BigDecimal.ZERO
        else:
            if self.X.compareTo(BigDecimal.valueOf(14927)) == -1:
                self.Y = self.X.subtract(self.GFB).divide(Lst2023.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
                ...
                if self.X.compareTo(BigDecimal.valueOf(58597)) == -1:
                    self.Y = self.X.subtract(BigDecimal.valueOf(14926)).divide(Lst2023.ZAHL10000, 6, BigDecimal.ROUND_DOWN)
